...
public class View extends Application {
        ...
        public void start(Stage primaryStage) throws Exception {
            ...
            openKeywordsWindowBtn.setOnAction(keywordWindowOpenEvent -> {
                ...
                try {
                    ...
                    // if the keywordTextField has at least one character in it, the button to add a keyword will be enabled, other wise it will be disabled
                    ...
                    // if a keyword is selected the keywordTextField is updated with the keyword, and the deleteKeywordBtn is enabled
                    ...
                    addKeywordBtn.setOnAction(add -> {
                        try {
                            ...
                            String KeywordGetTextValue = keywordTextField.getText();
                            //If the text field is not empty, the keyword can be added
                            if (!KeywordGetTextValue.isEmpty()) {
                                try {
                                    //controller.addKeyword(new Keyword(KeywordGetTextValue));
                                    controller.addKeyword(KeywordGetTextValue);
                                    controller.startKeywordTableQuery();
            ...
            /*
             * the id, the title and the author TextFields get listeners
             * if all three textFields have changed, user input has happened, the add button will be enabled
             * all three call the updateAddBtn method
             */
             ...
             * gets the id, the title and the author from the textfields that the user inputs
             * starts the query to update the documentTable
             * opens two dialog windows depending, depending on the input of the ID
             * throws dialog if a wrong value is given
             * no negative values or Strings are allowed
             * the ID 0 is allowed. Counting starts from 0!
             */
            addDocumentBtn.setOnAction(c -> {
                try {
                    Integer intIdValue;
                    intIdValue = Integer.parseInt(idTextField.getText());
                    if (intIdValue < 0 ){
                        throw new NumberFormatException();
                    }else{
                    controller.addDocument(new Document(intIdValue, titleTextField.getText(), authorTextField.getText()));
                    controller.startDocumentTableQuery();
                    }
                } catch (NumberFormatException e) {
                    Alert errorDialog = new Alert(Alert.AlertType.WARNING);
                    errorDialog.setTitle("ID is not an Integer error");
                    errorDialog.setHeaderText(null);
                    errorDialog.setContentText("Non Integer values or negative values are not valid. " + "\n" + "Please select another ID!");
                    errorDialog.showAndWait();
            ...
            /*
             * adds a listener to the documentsTable
             * when a document is selected the values for the ID, the Title and the Author are written into the TextFields
             * also sets the text for the referenceTextArea
             * handles the openKeywordsWindowBtn and the openReferenceBtn as well, since both only need to be accessed when a document is selected
             */
    ...
    /**
     * this method updates the delete button and the keywordTextArea
     * if a document is selected the delete button will be enabled
     * the keywordIDArray and thr DisplayKeywordsArray are also accessed
     * the keywords connected with the selected document will be displayed
     * @param deleteDocumentBtn is the delete Button that will be enabled or disabled, depends on the selection status
     * @param keywordsTextArea is the TextArea that will be updated with the Keywords that are connected to the document
     */
    ...
    /**
     * update method for the Button on the Archive Tab
     * the button will only be enabled once every text field on the archive tab has content
     * @param shedField is the shed TextField
     * @param rackField is the rack TextField
     * @param folderField   is the folder TextField
     * @param archiveOKBtn is the Button in the archive Tab that will be enabled when every TextField has content in it
     */
